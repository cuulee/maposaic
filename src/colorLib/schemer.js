...
export const getScheme = function (mode, count, seed) {
  let c = {
      ...
      image: {
        bare:
          'http://www.thecolorapi.com/scheme?format=svg&named=false&hex=' +
          seed.hex.clean +
          '&mode=' +
          mode +
          '&count=' +
          count,
        named:
          'http://www.thecolorapi.com/scheme?format=svg&hex=' + seed.hex.clean + '&mode=' + mode + '&count=' + count,
      },
      _links: {
...
export const schemer = {
  ...
  base: {
    ...
    'rgb-to-hsl': function (values) {
      let a, d, d2, db, dg, dr, h, l, rgb, s, z
      rgb = schemer.bounds.validate('rgb', values)
    ...
    'rgb-to-hsv': function (values) {
      let a, d, d2, db, dg, dr, h, rgb, s, v, z
      rgb = schemer.bounds.validate('rgb', values)
      a = Math.min(rgb.r, rgb.g, rgb.b)
      z = Math.max(rgb.r, rgb.g, rgb.b)
      d = z - a
  ...
  stringlify: {
    ...
    cmyk: function (values, tag) {
      ...
      return schemer.stringlify.tags(
        tag,
  ...
  scheme: {
    ...
    generate: function (mode, colors, color) {
      ...
      color = color || { h: 0.185, s: 0.5, l: 0.49 }
      ...
      for (groupIndex = _l = 0, _len3 = scheme.length; _l < _len3; groupIndex = ++_l) {
        group = scheme[groupIndex]
        for (
          colorCount = _m = 1, _ref = counts[groupIndex];
          1 <= _ref ? _m <= _ref : _m >= _ref;
          colorCount = 1 <= _ref ? ++_m : --_m
        ) {
